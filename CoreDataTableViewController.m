...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForManagedObject:(NSManagedObject *)managedObject IndexPath:(NSIndexPath *)indexPath
{
    static NSString *ReuseIdentifier = @"CoreDataTableViewCell";
	
	News *news = (News *)managedObject;
    
    NewsTableViewCell *cell = (NewsTableViewCell *)[tableView dequeueReusableCellWithIdentifier:ReuseIdentifier];
    if (cell == nil) {
		UITableViewCellStyle cellStyle = self.subtitleKey ? UITableViewCellStyleSubtitle : UITableViewCellStyleDefault;
        cell = [[[NewsTableViewCell alloc] initWithStyle:cellStyle reuseIdentifier:ReuseIdentifier] autorelease];
    } 

	NSString *cellTitle = [managedObject valueForKey:self.titleKey];
	NSString *cellTitleKey = [managedObject valueForKey:self.subtitleKey];
	UITableViewCellAccessoryType accessType = [self accessoryTypeForManagedObject:managedObject];

	if (self.titleKey) cell.textLabel.text = cellTitle;
	if (self.subtitleKey) cell.detailTextLabel.text = cellTitleKey;
	cell.accessoryType = accessType;
	cell.entryID = news.entryID;
	
	/*UIImage *thumnail = [self.imageCache objectForKey:indexPath];
	if (thumnail) {
		cell.imageView.image = thumnail;
	} else {*/
		
		cell.thumbnailURL = news.thumbnailURL;
		//[self startIconDownload:managedObject forIndexPath:indexPath];

		cell.imageView.image = [UIImage imageNamed:@"dog1.gif"];
	//}
...
{    
	return [self tableView:tableView cellForManagedObject:[[self fetchedResultsControllerForTableView:tableView] objectAtIndexPath:indexPath] IndexPath:indexPath];
}
